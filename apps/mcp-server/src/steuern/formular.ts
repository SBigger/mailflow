/**
 * Kennzahlen → Felder der Smartis-Steuerformulare (src/forms/*.js).
 *
 * Die Feld-IDs entsprechen exakt den `id`-Werten in den Formulardefinitionen
 * zh_500.js, sg_jp1b.js und tg_50i.js. Werte, die das Formular selbst rechnet
 * (ZH `computed`), werden NICHT gesetzt.
 *
 * Es werden nur Felder vorgeschlagen, die sich aus der Jahresrechnung
 * ableiten lassen. Steuerliche Korrekturen (Aufrechnungen, Abzuege,
 * Beteiligungsabzug, Ausscheidung) bleiben manuelle Eingaben.
 */
...
export interface Stammdaten {
  ...
  gj_von: string; // ISO
  gj_bis: string; // ISO
  vertreter_artis?: boolean;
  gemeinde_zh?: string | null; // Name aus ZH_GEMEINDEN, falls bekannt
}

export interface Verlustvortrag {
  /** steuerlich verrechenbare Vorjahresverluste (positiv) */
  betrag: number;
  /** Einzeljahre fuer SG Seite 4 */
  jahre?: { jahr: number; betrag: number }[];
  quelle: string;
}

export type Felder = Record<string, unknown>;

const leerWennNull = (n: number): number | "" => (Math.abs(n) < 0.005 ? "" : Math.round(n * 100) / 100);
const pos = (n: number): number | "" => (n > 0.005 ? Math.round(n * 100) / 100 : "");

function stammfelder(st: Stammdaten): Felder {
  return {
...
export function felderZH(k: Kennzahlen, st: Stammdaten, vv: Verlustvortrag | null): Felder {
  ...
  const f: Felder = {
    ...
    gv_dividende_pct: k.aktienkapital > 0 && k.gewinnverwendung.dividende > 0
      ? Math.round((k.gewinnverwendung.dividende / k.aktienkapital) * 10000) / 100 : "",
    ...
    // C. Eigenkapital (nach Gewinnverwendung)
  ...
  if (n.uebrige_reserve > 0.005) { f.ek_bez_135 = "Uebrige Reserven"; f.ek_betrag_135 = pos(n.uebrige_reserve); }
...
export function felderSG(k: Kennzahlen, st: Stammdaten, vv: Verlustvortrag | null): Felder {
  const n = k.ek_nach_verwendung;
  ...
  const ekBilanz = n.total - n.versteuerte_stille_reserven; // Code 530: EK laut Jahresrechnung nach Gewinnverwendung
  const f: Felder = {
    ...stammfelder(st),
    sitzgemeinde: st.ort ?? "",
    register_nr: st.register_nr ?? "",
    abschluss_vom: st.gj_bis,
    // Gewinnsteuer (Codes 100/150/250/260/270)
    reingewinn_buch: leerWennNull(k.jahresergebnis),
    ...
    // Kapitalsteuer (Codes 512/518/530/540/555/560/570/580)
  ...
  if (vv?.jahre?.length) {
    vv.jahre.slice(0, 7).forEach((j, i) => {
      f[`verlust_jahr_${i + 1}`] = String(j.jahr);
      f[`verlust_betrag_${i + 1}`] = pos(j.betrag);
    });
...
export function felderTG(k: Kennzahlen, st: Stammdaten, vv: Verlustvortrag | null): Felder {
  const n = k.ek_nach_verwendung;
  const g = k.gewinnverwendung;
  const verlust = vv?.betrag ?? 0;
  const verrechenbar = Math.min(Math.max(k.jahresergebnis, 0), verlust);
  const steuerbar = k.jahresergebnis - verrechenbar;
  const ekTotal = n.total;
  const f: Felder = {
    ...stammfelder(st),
    hr_nummer: st.uid ?? "",
    reingewinn_buch: leerWennNull(k.jahresergebnis),
    ...
    verlustvortrag_abzug: pos(verrechenbar),
    steuerbarer_gewinn_kt: leerWennNull(steuerbar),
    // Kapitalsteuer Ziffern 13/14/16/18/20
    ...
    eigenkapital_total: leerWennNull(ekTotal),
    steuerbares_kapital: pos(Math.max(ekTotal, n.aktienkapital)),
    kapital_tg: pos(Math.max(ekTotal, n.aktienkapital)),
    // Bilanzgewinn / Gewinnverwendung Ziffer 12
...
export function felderFuerKanton(kanton: Kanton, k: Kennzahlen, st: Stammdaten, vv: Verlustvortrag | null): Felder {
  switch (kanton) {
...
/**
 * Zusammenfuehren mit bereits gespeicherten Feldern.
 * Standard: bestehende, von Hand erfasste Werte bleiben stehen (Konflikte werden gemeldet).
 */
export function zusammenfuehren(
  bestehend: Felder,
  vorschlag: Felder,
  ueberschreiben: boolean,
): { felder: Felder; neu: string[]; geaendert: string[]; konflikte: { feld: string; bestehend: unknown; vorschlag: unknown }[] } {
  const felder: Felder = { ...bestehend };
  const neu: string[] = [];
  const geaendert: string[] = [];
  const konflikte: { feld: string; bestehend: unknown; vorschlag: unknown }[] = [];
