 ...
 * Operationen auf "den aktuellen Kunden" defaulten auf CUSTOMER_ID.
 */
...
/** Liefert die explizit uebergebene ID oder faellt auf CUSTOMER_ID zurueck. */
function resolveCustomerId(explicit?: string): string {
  if (explicit) return explicit;
  if (config.customerId) return config.customerId;
  throw new ToolError("Keine customer_id angegeben und CUSTOMER_ID ist nicht konfiguriert.");
}
...
export function registerCustomerTools(server: McpServer): void {
  ...
  registerTool(server, {
    ...
    handler: async (args) => {
      const id = resolveCustomerId(args.id);
      const data = unwrap(
        await supabase
          .from("customers")
  ...
  registerTool(server, {
    ...
    handler: async (args) => {
      ...
      const id = resolveCustomerId(explicit);
