...
export default function Dokumente() {
  ...
  const handleCheckout = async (doc) => {
    try {
      ...
      queryClient.setQueryData(["dokumente-all"], (old) => (old || []).map(d => d.id === freshDoc.id ? { ...freshDoc, checked_out_by: authUser.id, checked_out_by_name: user?.full_name || authUser.email, checked_out_at: new Date().toISOString() } : d));
      queryClient.invalidateQueries({ queryKey: ["dokumente-all"] });
      queryClient.invalidateQueries({ queryKey: ["dokumente"] });
