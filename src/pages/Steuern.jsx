...
import {
  ...
  Building2, X, Wrench, Check, CheckCircle2, Star, Trash2, Paperclip, ExternalLink,
} from 'lucide-react';
...
// ── BelegePicker (Veranlagungs-Dokumente verknüpfen) ─────────────────────────
// Pattern aus Abschlussdokumentation.jsx übernommen: Modal mit Filter
// (customer_id, optional year, optional category) + Chip-Liste der verknüpften
// Dokumente. Speichert nur die Doc-Refs (id, name, filename, storage_path,
// year, category, file_type) im felder-JSONB.
...
function BelegePicker({ feld, value, onChange, customerId, steuerjahr }) {
  const belege = Array.isArray(value) ? value : [];
  const [showPicker, setShowPicker] = useState(false);
  const [search, setSearch]         = useState('');
  const [docs, setDocs]             = useState([]);
  const [loading, setLoading]       = useState(false);
  const [yearFilter, setYearFilter] = useState(steuerjahr || null); // null = alle Jahre
  const linkedIds = new Set(belege.map(b => b.id));

  // Beim Wechsel von Kunde/Jahr docs leeren → erzwingt Reload
  ...
  useEffect(() => {
    ...
    let q = supabase.from('dokumente')
      ...
      .limit(500);
    if (feld?.kategorie) q = q.eq('category', feld.kategorie);
    if (yearFilter)      q = q.eq('year', yearFilter);
    q.then(({ data }) => { setDocs(data || []); setLoading(false); });
  }, [showPicker, customerId, yearFilter, feld?.kategorie, docs.length]);
  ...
  const pickerModal = showPicker && createPortal(
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, backgroundColor: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      ...
      <div style={{ backgroundColor: '#fff', borderRadius: 14, boxShadow: '0 24px 64px rgba(0,0,0,0.22)', width: 'min(820px, 94vw)', maxHeight: '82vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ padding: '14px 18px', borderBottom: `1px solid ${C.panelBdr}`, display: 'flex', alignItems: 'center', gap: 12 }}>
          <Paperclip className="w-4 h-4" style={{ color: C.accent }} />
          <span style={{ fontSize: 14, fontWeight: 700, color: C.heading, flex: 1 }}>Veranlagung verknüpfen</span>
          <span style={{ fontSize: 12, color: C.sub }}>
            {feld?.kategorie ? `Kategorie: ${feld.kategorie}` : 'Alle Kategorien'} · {yearFilter ? `Jahr ${yearFilter}` : 'Alle Jahre'} · {filtered.length} Dokumente
          </span>
          <button onClick={() => setShowPicker(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.muted, padding: '0 4px' }}>
            <X className="w-4 h-4" />
          </button>
        </div>
        <div style={{ padding: '10px 18px', borderBottom: `1px solid ${C.panelBdr}`, display: 'flex', gap: 8, alignItems: 'center' }}>
          <input autoFocus value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Dokument suchen…"
            style={{ flex: 1, fontSize: 13, padding: '7px 12px', borderRadius: 8, border: `1px solid ${C.panelBdr}`, outline: 'none' }} />
          <button onClick={() => setYearFilter(yearFilter ? null : (steuerjahr || null))}
            title={yearFilter ? 'Alle Jahre zeigen' : `Nur Jahr ${steuerjahr}`}
            style={{ fontSize: 11, padding: '6px 10px', borderRadius: 6, border: `1px solid ${C.panelBdr}`, background: yearFilter ? C.accentBg : '#fff', color: yearFilter ? C.accent : C.sub, cursor: 'pointer', fontWeight: 600 }}>
            {yearFilter ? `Jahr ${yearFilter}` : 'Alle Jahre'}
          </button>
        </div>
        <div style={{ overflowY: 'auto', flex: 1 }}>
          {loading
            ? <div style={{ padding: 32, textAlign: 'center', fontSize: 13, color: C.sub }}>Lädt…</div>
            : filtered.length === 0
              ? <div style={{ padding: 32, textAlign: 'center', fontSize: 13, color: C.sub }}>Keine Dokumente gefunden</div>
              : filtered.map(doc => {
                  ...
                  return (
                    <div key={doc.id} onClick={() => { if (!already) addBeleg(doc); }}
                      style={{ padding: '10px 18px', cursor: already ? 'default' : 'pointer', opacity: already ? 0.5 : 1, borderBottom: `1px solid ${C.panelBdr}`, display: 'flex', alignItems: 'center', gap: 12, transition: 'background 0.1s' }}
                      onMouseEnter={e => { if (!already) e.currentTarget.style.backgroundColor = C.accent + '10'; }}
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 13, color: C.heading, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{doc.name}</div>
                        <div style={{ fontSize: 11, color: C.sub, marginTop: 2 }}>{doc.filename}</div>
                      </div>
                      <div style={{ flexShrink: 0, textAlign: 'right' }}>
                        {doc.year && <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 10, backgroundColor: C.accent + '15', color: C.accent }}>{doc.year}</span>}
                        {doc.category && <div style={{ fontSize: 10, color: C.sub, marginTop: 3 }}>{doc.category}</div>}
                      </div>
                      {already
                        ? <CheckCircle2 className="w-3.5 h-3.5" style={{ color: '#16a34a', flexShrink: 0 }} />
                        : <Plus className="w-3.5 h-3.5" style={{ color: C.accent, flexShrink: 0 }} />}
                    </div>
                  );
                })
          }
    ...
    <div style={{ width: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
        <span style={{ fontSize: 11, color: C.sub }}>
          {belege.length === 0 ? 'Noch keine Veranlagung verknüpft' : `${belege.length} Dokument${belege.length === 1 ? '' : 'e'} verknüpft`}
        </span>
        <button
          type="button"
          ...
          style={{
            ...
            backgroundColor: C.accent + '14', border: `1px solid ${C.accent}40`, color: C.accent,
            opacity: customerId ? 1 : 0.5,
          }}>
          <Plus className="w-3 h-3" /> Veranlagung verknüpfen
        </button>
      </div>
      {belege.length > 0 && (
          ...
          {belege.map(b => {
            ...
            return (
              <div key={b.id} style={{
                ...
                borderRadius: 6, backgroundColor: C.inputBg, border: `1px solid ${C.panelBdr}`, fontSize: 11,
              }}>
                ...
                <span style={{ maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: C.heading }} title={b.name}>{b.name}</span>
                {b.year && <span style={{ color: C.sub, fontSize: 10, flexShrink: 0 }}>{b.year}</span>}
                <button onClick={() => openDoc(b)} title="Öffnen"
                  style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.accent, padding: '0 2px', display: 'flex', alignItems: 'center' }}>
                  <ExternalLink className="w-3 h-3" />
                </button>
                <button onClick={() => removeBeleg(b.id)} title="Verknüpfung entfernen"
                  style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.muted, fontSize: 14, padding: '0 1px', opacity: 0.55, lineHeight: 1 }}>×</button>
              </div>
            );
          })}
        </div>
          ...
          <div key={section.id}>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-3">
              {section.felder.map(feld => {
                ...
                return (
                <div key={feld.id} className={(feld.typ === 'textarea' || feld.typ === 'belege') ? 'col-span-2' : ''}>
                  ...
                  {feld.typ === 'belege'
                    ? <BelegePicker feld={feld} value={felder[feld.id]} onChange={val => set(feld.id, val)} customerId={kunde.id} steuerjahr={steuerjahr} />
