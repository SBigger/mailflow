...
async function spCall(jwt, body) {
    ...
    if (!res.ok) {
        ...
        throw new Error(e.error || `SharePoint Fehler ${res.status}`);
    }
    return res.json();
}
            ...
            <div style={card}>
                <div style={{display: "flex", alignItems: "center", gap: 10, marginBottom: 18}}>
                    ...
                    <div style={{flex: 1}}>
                        ...
                        <div style={{
                            color: s.textMuted,
                            fontSize: 12
                        }}>{links.length} Link{links.length !== 1 ? "s" : ""}</div>
                    </div>
                    <button onClick={onClose} style={{
                        background: "none",
                        border: "none",
                ...
                <div style={{flex: 1, overflowY: "auto", margin: "0 -8px", padding: "0 8px"}}>
                    ...
                    {!loading && links.length === 0 &&
                        <div style={{textAlign: "center", color: s.textMuted, padding: 40}}>Keine Links erstellt.</div>}
...
function UploadDialog({customers, preCustomer, preFile, allTags, onCancel, onUpload, s, border, accent}) {
    ...
    const handleUpload = async () => {
        const newErrors = {};
        if (!file || !custId || !name.trim()) {
            toast.error("Bitte Datei, Kunde und Name ausf\u00fcllen");
            return;
        }
        ...
        if (!tagIds || tagIds.length === 0) {
            newErrors.tags = "Bitte mindestens einen Tag ausw\u00e4hlen.";
        }
        ...
        try {
            ...
            if (newDoc?.id) {
                await supabase.functions.invoke("process-document", {
                    ...
                    console.log("indexierung Failed: ", error)
                });
            }
            ...
            <div ref={dialogRef} style={{
                ...
                <div onMouseDown={onHeaderMouseDown} style={{
                    ...
                    <button onClick={onCancel}
                            style={{background: "none", border: "none", cursor: "pointer", color: s.textMuted}}><X
                        size={18}/></button>
                </div>
                <div style={{display: "flex", flexDirection: "column", gap: 10}}>
                    ...
                    <div
                        ...
                        style={{
                            border: "2px dashed " + (file ? accent : border),
                            borderRadius: 8,
                            padding: 14,
                            textAlign: "center",
                            cursor: "pointer",
                    ...
                    {/* Kunde */}
                    <div>
                        <label style={{fontSize: 12, color: s.textMuted, display: "block", marginBottom: 3}}>Kunde
                            *</label>
                    ...
                    {/* Name */}
                    <div>
                        <label style={{fontSize: 12, color: s.textMuted, display: "block", marginBottom: 3}}>Anzeigename
                            *</label>
                    ...
                    {/* Tags */}
                    <div>
                        <label style={{fontSize: 12, color: s.textMuted, display: "block", marginBottom: 3}}>Tags
                            *</label>
                        <TagSelectWidget value={tagIds} onChange={(v) => {
                            setTagIds(v);
                    ...
                    <div style={{display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10}}>
                        <div>
                            <label style={{fontSize: 12, color: s.textMuted, display: "block", marginBottom: 3}}>Kategorie
                                *</label>
                            <select value={category} onChange={e => {
                                ...
                                ...inp,
                                cursor: "pointer",
                                borderColor: errors.category ? "#ef4444" : (s.inputBorder || border)
                            }}>
                                <option value="">-- Kategorie wählen --</option>
                                {CATEGORIES.map(c => <option key={c.key} value={c.key}>{c.icon} {c.label}</option>)}
                            </select>
                            {errors.category &&
                                <div style={{color: "#ef4444", fontSize: 11, marginTop: 3}}>{errors.category}</div>}
                        </div>
                        <div>
                            <label style={{fontSize: 12, color: s.textMuted, display: "block", marginBottom: 3}}>Jahr
                                *</label>
                    ...
                    {/* Notiz */}
                    <div>
                        <label style={{fontSize: 12, color: s.textMuted, display: "block", marginBottom: 3}}>Notiz
                            (optional)</label>
                        <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2}
                                  style={{...inp, resize: "none"}} placeholder="Kurze Bemerkung..."/>
                    </div>
                </div>
                <div style={{display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 16}}>
                    <Button variant="outline" onClick={onCancel}
                            style={{color: s.textMuted, borderColor: border}}>Abbrechen</Button>
                    <Button onClick={handleUpload} disabled={uploading || !file || !custId || !year}
...
function EditDialog({doc, allTags, customers = [], onCancel, onSave, s, border, accent, mode = "edit"}) {
    ...
    const applyAutoName = () => {
        if (!suggestedName) {
            toast.error("Erst Tags auswaehlen oder Text eingeben");
            return;
        }
        setName(suggestedName);
    };
    ...
    const handleSave = async () => {
        if (!name.trim() || !year || isNaN(parseInt(year))) {
            toast.error("Name und Jahr sind Pflicht");
            return;
        }
        if (!customerId) {
        ...
        <div style={{
            ...
            <div style={{
                ...
                <div style={{display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16}}>
                    <h3 style={{color: s.textMain, fontSize: 14, fontWeight: 700}}>
                        ...
                        {isCopy && <span style={{
                            ...
                            padding: "2px 7px",
                            borderRadius: 6
                        }}>Datei wird physisch dupliziert</span>}
                    </h3>
                    <button onClick={onCancel}
                            style={{background: "none", border: "none", cursor: "pointer", color: s.textMuted}}><X
                        size={18}/></button>
                ...
                <div style={{display: "flex", flexDirection: "column", gap: 10}}>
                    <div>
                        <label
                            style={{fontSize: 12, color: s.textMuted, display: "block", marginBottom: 3}}>Datei</label>
                        <div style={{
                            fontSize: 12,
                            color: s.textMuted,
                            padding: "5px 8px",
                    ...
                    <div>
                        <label style={{fontSize: 12, color: s.textMuted, display: "block", marginBottom: 3}}>Unternehmen
                            *</label>
                        <select value={customerId} onChange={e => setCustomerId(e.target.value)}
                                style={{
                                    ...inp,
                                    cursor: "pointer",
                                    borderColor: !customerId ? "#ef4444" : (s.inputBorder || border)
                                }}>
                    ...
                    <div style={{display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10}}>
                        ...
                        <div>
                            <label style={{fontSize: 12, color: s.textMuted, display: "block", marginBottom: 3}}>Jahr
                                *</label>
                            <input type="number" value={year} min="2000" max="2099"
                                   onChange={e => setYear(e.target.value)}
                                   style={{...inp, borderColor: !year ? "#ef4444" : (s.inputBorder || border)}}/>
                        </div>
                    </div>
                    <div>
                        <label
                            style={{fontSize: 12, color: s.textMuted, display: "block", marginBottom: 3}}>Tags</label>
                        <TagSelectWidget value={tagIds} onChange={setTagIds} allTags={filteredTags} s={s}
                                         border={border} accent={accent}/>
                    </div>
                    <div>
                        <label
                            style={{fontSize: 12, color: s.textMuted, display: "block", marginBottom: 3}}>Notiz</label>
                        <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2}
                                  style={{...inp, resize: "none"}} placeholder="Kurze Bemerkung..."/>
                    </div>
                </div>
...
export default function Dokumente() {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            ...
            try {
                ...
                const lowestScore = data.results[data.results.length - 1]?.combined_score || 0;

                const UIResults = data.results.map(item => {
                    // Der echte Bereich zwischen bestem und schlechtestem Treffer dieser spezifischen Suche
                    const totalRange = highestScore - lowestScore;

                    let relevancePercentage = 0;
                    if (totalRange > 0) {
                        // Berechnet die Position des aktuellen Scores exakt innerhalb dieser Spanne (0% - 100%)
                        relevancePercentage = Math.round(((item.combined_score - lowestScore) / totalRange) * 100);
                    } else {
                        // Falls alle Dokumente exakt denselben Score haben (totalRange === 0)
    ...
    useEffect(() => {
        const check = setInterval(() => {
            ...
            try {
                ...
                console.error("Excel-Upload Fehler:", e);
            }
        }, 500);
    ...
    const tagLabel = (id) => {
        const t = getTag(id);
        if (!t) return null;
        const p = t.parent_id ? getTag(t.parent_id) : null;
        return p ? `${p.name} / ${t.name}` : t.name;
    };
    const tagColor = (id) => {
        const t = getTag(id);
        if (!t) return accent;
        return (t.parent_id ? getTag(t.parent_id)?.color : null) || t.color || accent;
    };
    ...
    const abschlussTree = useMemo(() => {
        ...
        filtered.forEach(doc => {
            ...
            if (!ids.length) {
                ...
                if (!parentMap.has("__none__")) parentMap.set("__none__", {
                    tag: {
                        id: "__none__",
                        name: "Ohne Tag",
                        sort_order: 9999
                ...
                if (!pm.children.has("__direct__")) pm.children.set("__direct__", {
                    tag: {
                        id: "__direct__",
                        name: "Allgemein",
                        sort_order: 0
    ...
    const breadcrumb = useMemo(() => {
        ...
        if (selCat) {
            const c = CATEGORIES.find(x => x.key === selCat);
            if (c) parts.push(c.icon + " " + c.label);
        }
    ...
    useEffect(() => {
        ...
        if (isLoading) {
            const startTime = Date.now(); // Startzeitpunkt in Millisekunden festhalten
            setMs(0);

            // Aktualisiert die Anzeige alle 50 Millisekunden für ein flüssiges Bild
    ...
    const selectCustomer = (id) => {
        ...
        setSelCat(null);
        setSelYear(null);
        setExpandedC(p => ({...p, [id]: true}));
    };
    const selectCat = (cid, ck) => {
        setSelCustomerId(cid);
        setSelCat(ck);
        setSelYear(null);
        setExpandedCat(p => ({...p, [cid + "_" + ck]: true}));
    };
            ...
            <div style={{
                    ...
                    {[['alle', 'Alle Dokumente'], ['ausgecheckt', 'Ausgecheckt']].map(([key, label]) => (
                        <button key={key} onClick={() => setPageTab(key)}
                                style={{
                                    ...
                                    cursor: "pointer",
                                    fontSize: 12,
                                    fontWeight: pageTab === key ? 600 : 400,
                                    background: pageTab === key ? accent : "transparent",
                                    color: pageTab === key ? "#fff" : s.textMuted,
                ...
                <div style={{position: "relative"}}>
                    <Search size={13} style={{
                        ...
                        color: ftSearching ? accent : s.textMuted,
                        pointerEvents: "none"
                    }}/>
                    <input
                        value={ftSearch}
                        ...
                        style={{
                            background: s.inputBg,
                            border: "1px solid " + (ftSearch ? accent : border),
                            color: s.textMain,
                            borderRadius: 8,
                ...
                <Button onClick={() => setShowShareLinks(true)} style={{
                ...
                <div style={{flex: 1, overflowY: "auto", padding: "12px 20px"}}>
                    ...
                    <div style={{display: "flex", flexDirection: "column", gap: 6}}>
                        {ftResults.map((doc, index) => {
                            ...
                            return (
                                <div key={doc.chunk_id || doc.id || index}
                                     style={{
                                         display: "flex",
                                         alignItems: "center",
                                         gap: 10,
                                         ...
                                         transition: "background 0.1s", ...(doc.id === highlightDocId ? {
                                             boxShadow: "0 0 0 2px " + accent + "88 inset",
                                             background: accent + "12",
                                             borderRadius: 6
                                         } : {})
                                     }}
                                    ...
                                    <span onClick={() => {
                                        ...
                                        if (!doc.checked_out_by) {
                                            ...
                                            toast.error('URL nicht verfuegbar.');
                                        }
                                    }} style={{
                                        background: fi.color,
                                        color: "#fff",
                                    ...
                                    <div style={{flex: 1, minWidth: 0}}>
                                        ...
                                        <div className="flex align-center justify-between text-sm text-gray-600 bg-gray-50 p-1 rounded border-l-4 whitespace-pre-wrap leading-relaxed">
                                            <div>
                                                <p>Volltext-score: {parseFloat(Number(doc.full_text).toFixed(6))}</p>
                                                <p>Vector-score: {parseFloat(Number(doc.vektor_text).toFixed(6))}</p>
                                            </div>
                                            <HighlightedText text={doc.extracted_text} searchQuery={ftSearch}/>
                                        ...
                                        {isCheckedOut && (
                                            <span
                                                title={"Ausgecheckt von " + doc.checked_out_by_name + " am " + (doc.checked_out_at ? new Date(doc.checked_out_at).toLocaleDateString("de-CH") : "")}
                                                style={{
                                                    fontSize: 10, color: isMyCheckout ? accent : "#f59e0b",
                          ...
                          <Lock size={9}/>
                              ...
                              <span style={{width: 5, height: 5, borderRadius: "50%", background: col}}/>
                                            ...
                                            {ids.length > 4 && <span
                                                style={{fontSize: 10, color: s.textMuted}}>+{ids.length - 4}</span>}
                                            {doc.notes && <span style={{
                                                fontSize: 10,
                                                color: s.textMuted,
                                                fontStyle: "italic"
                                            }}>{doc.notes}</span>}
                        ...
                        {new Date(doc.updated_at || doc.created_at).toLocaleDateString("de-CH", {
                            day: "2-digit",
                            month: "2-digit",
                            year: "2-digit"
                        })}
                                    ...
                                    {/* Download */}
                                    <button onClick={() => {
                                        animateBtn(`dl-${doc.id}`);
                                        downloadDoc(doc, true);
                                    }}
                                            title="Herunterladen" style={{
                                        background: "none",
                                        border: "none",
                                        cursor: "pointer",
                                        color: accent,
                                        ...
                                        transform: clickedBtns[`dl-${doc.id}`] ? "scale(0.75)" : "scale(1)"
                                    }}>
                                        <Download size={15}/>
                                    </button>
                                    {/* Share */}
                                    <button
                                        ...
                                        style={{
                                            ...
                                            flexShrink: 0,
                                            transition: "transform 0.15s",
                                            transform: clickedBtns[`sh-${doc.id}`] ? "scale(0.75)" : "scale(1)"
                                        }}>
                                        <Link2 size={15}/>
                                    </button>
                                    {/* Loeschen */}
                                    <button onClick={() => !lockedByOther && handleDelete(doc)}
                                            title={lockedByOther ? "Gesperrt – kann nicht geloescht werden" : "Loeschen"}
                                            style={{
                                                background: clickedBtns[`del-${doc.id}`] ? "#ef444420" : "none",
                                                border: "none",
                                                cursor: lockedByOther ? "not-allowed" : "pointer",
                                                color: lockedByOther ? s.textMuted + "44" : "#ef4444",
                                                display: "flex",
                                                alignItems: "center",
                                                padding: 4,
                                                borderRadius: 4,
                                                ...
                                                transform: clickedBtns[`del-${doc.id}`] ? "scale(0.75)" : "scale(1)"
                                            }}>
                                        <Trash2 size={15}/>
                                    </button>
                                </div>
                    ...
                    {myCheckedOutDocs.length === 0 ? (
                        ...
                        <div style={{display: "flex", flexDirection: "column", gap: 10, maxWidth: 860}}>
                            <div style={{fontSize: 12, color: s.textMuted, marginBottom: 4}}>
                                {myCheckedOutDocs.length} Dokument{myCheckedOutDocs.length !== 1 ? "e" : ""} von dir
                                ausgecheckt
                            </div>
                ...
                <div style={{flex: 1, display: "flex", overflow: "hidden"}}>
                    ...
                    <div style={{
                        ...
                        <div style={{padding: "8px 10px", borderBottom: "1px solid " + border, position: "relative"}}>
                            ...
                            <input value={custSearch} onChange={e => setCustSearch(e.target.value)}
                                   placeholder="Kunde suchen... (oder einfach lostippen)"
                                   style={{
                                       background: s.inputBg,
                                       border: "1px solid " + border,
                                       color: s.textMain,
                        ...
                        <div onClick={() => {
                            ...
                            setSelYear(null);
                             ...
                             style={{
                                 ...treeItem,
                                 margin: "4px 6px",
                                 background: !selCustomerId ? s.selBg : "transparent",
                                 color: !selCustomerId ? accent : s.textMain,
                                 fontWeight: !selCustomerId ? 600 : 400
                             }}>
                        ...
                        <div style={{flex: 1, overflowY: "auto", paddingBottom: 8}}>
                            {isLoading ? <div style={{padding: 16, color: s.textMuted, fontSize: 12}}>
                                    Lädt seit {ms} ms...
                                </div>
                                : tree.length === 0 ?
                                        ...
                                        return (
                                            <div key={cust.id} data-cust-id={cust.id} style={{margin: "1px 6px"}}>
                                                <div onClick={() => selectCustomer(cust.id)}
                                                     style={{
                                                         ...treeItem,
                                                         background: selCustomerId === cust.id ? s.selBg + (isCustSel ? "" : "88") : "transparent",
                                                         color: selCustomerId === cust.id ? accent : s.textMain,
                                                         fontWeight: selCustomerId === cust.id ? 600 : 400
                                                     }}>
                      <span onClick={e => {
                          e.stopPropagation();
                          setExpandedC(p => ({...p, [cust.id]: !p[cust.id]}));
                      }}
                            style={{
                                display: "flex",
                                alignItems: "center",
                        ...
                        {isExp ? <ChevronDown size={11}/> : <ChevronRight size={11}/>}
                                                    ...
                                                    <span
                                                        style={{fontSize: 13}}>{isExp ? "\uD83D\uDCC2" : "\uD83D\uDCC1"}</span>
                                                    <span style={{
                                                        flex: 1,
                                                        overflow: "hidden",
                                                        textOverflow: "ellipsis",
                                                        whiteSpace: "nowrap"
                                                    }}>{cust.company_name}</span>
                                                    <span style={{
                                                        fontSize: 10,
                                                        color: s.textMuted,
                                                        background: border,
                                                    ...
                                                    <span onClick={e => {
                                                        e.stopPropagation();
                                                        downloadFolder(cust.id, null, null);
                                                    }}
                                                    ...
                                                    return (
                                                        <div key={cat.key} style={{paddingLeft: 14}}>
                                                            <div onClick={() => selectCat(cust.id, cat.key)}
                                                                 style={{
                                                                     ...treeItem,
                                                                     background: catSel ? s.selBg : "transparent",
                                                                     color: (selCustomerId === cust.id && selCat === cat.key) ? accent : s.textMain,
                                                                     fontWeight: catSel ? 600 : 400
                                                                 }}>
                            <span onClick={e => {
                                e.stopPropagation();
                                setExpandedCat(p => ({...p, [ck]: !p[ck]}));
                            }}
                                  style={{
                                      display: "flex",
                                      alignItems: "center",
                              ...
                              {cat.years.length > 0 || cat.noYear > 0
                                  ...
                                  : <span style={{width: 11}}/>}
                                                                ...
                                                                <span onClick={e => {
                                                                    e.stopPropagation();
                                                                    downloadFolder(cust.id, cat.key, null);
                                                                }}
                              ...
                              <Download size={11}/>
                                                                ...
                                                                <span onClick={e => {
                                                                    ...
                                                                    setShareDialog({
                                                                        ...
                                                                        customer_id: cust.id,
                                                                        category: cat.key,
                                                                        name: cust.company_name + " – " + cat.label
                                                                    });
                                                                }}
                                                            ...
                                                            {catExp && (
                                                                <div style={{paddingLeft: 14}}>
                                                                    {cat.years.map(yr => {
                                                                        ...
                                                                        return (
                                                                            <div key={yr} onClick={() => {
                                                                                setSelCustomerId(cust.id);
                                                                                setSelCat(cat.key);
                                                                                setSelYear(yr);
                                                                            }}
                                                                                 style={{
                                                                                     ...treeItem,
                                                                                     background: yrSel ? s.selBg : "transparent",
                                                                                     color: yrSel ? accent : s.textMain,
                                                                                     fontWeight: yrSel ? 600 : 400
                                                                                 }}>
                                                                                <span
                                                                                ...
                                                                                <span onClick={e => {
                                                                                    e.stopPropagation();
                                                                                    downloadFolder(cust.id, cat.key, yr);
                                                                                }}
                                                                    ...
                                                                    {cat.noYear > 0 && (
                                                                        <div onClick={() => {
                                                                            ...
                                                                            setSelYear("__none__");
                                                                             ...
                                                                             style={{
                                                                                 ...treeItem,
                                                                                 background: (selCustomerId === cust.id && selCat === cat.key && selYear === "__none__") ? s.selBg : "transparent",
                                                                                 color: s.textMuted,
                                                                                 fontStyle: "italic"
                                                                             }}>
                    ...
                    <div
                        ...
                        onDragOver={e => {
                            e.preventDefault();
                            e.stopPropagation();
                            if (selCustomerId) setDragOver(true);
                        }}
                        onDragEnter={e => {
                            e.preventDefault();
                            e.stopPropagation();
                        ...
                        onDrop={e => {
                            ...
                            setDragOver(false);
                            if (!selCustomerId) {
                                toast.error("Bitte zuerst einen Kunden in der Seitenleiste auswählen");
                                return;
                            }
                    ...
                    >
                        ...
                        {dragOver && (
                            <div style={{
                                ...
                                {selCustomer && <span style={{
                                    fontSize: 12,
                                    color: accent,
                                    opacity: 0.7
                                }}>für {selCustomer.company_name}</span>}
                            </div>
                        )}
                        {/* Topbar */}
                        <div style={{
                            ...
                            <div style={{
                                ...
                                {selCustomer ? (<>
                                    <span style={{
                                        ...
                                        whiteSpace: "nowrap",
                                        flexShrink: 1
                                    }}>{selCustomer.company_name}</span>
                                    <span onClick={() => setShareDialog({
                                        type: 'folder',
                                        customer_id: selCustomer.id,
                                        name: selCustomer.company_name
                                    })}
                                          title="Alle Dokumente dieses Kunden teilen" style={{
                                        cursor: "pointer",
                                        opacity: 0.5,
                                        flexShrink: 0,
                                        display: "flex"
                                    ...
                                    {selCat && (<>
                                        ...
                                        <span style={{
                                            whiteSpace: "nowrap",
                                            flexShrink: 0
                                        }}>{(CATEGORIES.find(x => x.key === selCat)?.icon || "") + " " + (CATEGORIES.find(x => x.key === selCat)?.label || selCat)}</span>
                                        <span onClick={() => setShareDialog({
                                            type: 'folder',
                                            customer_id: selCustomer.id,
                                            category: selCat,
                                            name: selCustomer.company_name + " – " + (CATEGORIES.find(x => x.key === selCat)?.label || selCat)
                                        })}
                                              title="Diese Kategorie teilen"
                                              style={{cursor: "pointer", opacity: 0.5, flexShrink: 0, display: "flex"}}><Link2
                                            size={12}/></span>
                                    </>)}
                                    {selYear && (<>
                                        <span style={{
                                        ...
                                        <span style={{
                                            whiteSpace: "nowrap",
                                            flexShrink: 0
                                        }}>{selYear === "__none__" ? "Kein Jahr" : selYear}</span>
                                        {selYear !== "__none__" && (
                                            <span onClick={() => setShareDialog({
                                                type: 'folder',
                                                customer_id: selCustomer.id,
                                                category: selCat,
                                                year: selYear,
                                                name: selCustomer.company_name + " – " + (CATEGORIES.find(x => x.key === selCat)?.label || selCat) + " " + selYear
                                            })}
                                                  title="Dieses Jahr teilen" style={{
                                                cursor: "pointer",
                                                opacity: 0.5,
                                                flexShrink: 0,
                                                display: "flex"
                                    ...
                                    {[{key: "normal", label: "Normal"}, {
                                        ...
                                        <button key={v.key} onClick={() => {
                                            ...
                                            setOpenFolders(new Set());
                                                ...
                                                style={{
                                                    background: viewMode === v.key ? accent : "none",
                                                    color: viewMode === v.key ? "#fff" : s.textMuted,
                                                    border: "none",
                                                    cursor: "pointer",
                                                    borderRadius: 4,
                                                    padding: "2px 10px",
                                ...
                                abschlussTree.map(({tag: parent, children}) => {
                                    ...
                                    return (
                                        <div key={parent.id}>
                                            {/* Parent-Ordner */}
                                            <div
                                                onClick={() => setOpenFolders(f => {
                                                    const n = new Set(f);
                                                    n.has(parent.id) ? n.delete(parent.id) : n.add(parent.id);
                                                    return n;
                                                })}
                                                style={{
                                                    display: "flex",
                                                    ...
                                                    cursor: "pointer",
                                                    background: isParentOpen ? pColor + "10" : "transparent",
                                                    ...
                                                    return (
                                                        <div key={child.id}>
                                                            ...
                                                            <div
                                                                onClick={() => setOpenFolders(f => {
                                                                    const n = new Set(f);
                                                                    n.has(childKey) ? n.delete(childKey) : n.add(childKey);
                                                                    return n;
                                                                })}
                                                                style={{
                                                                    display: "flex",
                                                                    ...
                                                                    cursor: "pointer",
                                                                    background: isChildOpen ? cColor + "08" : "transparent",
                                                                    transition: "background 0.1s"
                                                                }}
                                ...
                                {child.id === "__direct__" ? "" : child.name}
                                  ...
                                  <span onClick={() => {
                                        ...
                                        style={{
                                            ...
                                            cursor: "pointer"
                                                                        ...
                                                                        {isCheckedOut && (
                                                                            <span style={{
                                                                                fontSize: 10,
                                                                                color: isMyCheckout ? accent : "#f59e0b",
                                                                                background: isMyCheckout ? accent + "18" : "#f59e0b18",
                                                                                border: "1px solid " + (isMyCheckout ? accent + "44" : "#f59e0b44"),
                                                                                borderRadius: 8,
                                                                                padding: "1px 6px",
                                      ...
                                      <Lock size={9}/>{isMyCheckout ? "Ich" : doc.checked_out_by_name}
                                                                        ...
                                                                        {doc.created_at && (
                                                                            <span
                                                                                title={"Hochgeladen: " + new Date(doc.created_at).toLocaleString("de-CH")}
                                                                                style={{
                                                                                    fontSize: 10,
                                                                                    color: s.textMuted,
                                                                                    flexShrink: 0,
                                      ...
                                      {new Date(doc.created_at).toLocaleDateString("de-CH", {
                                          ...
                                          year: "2-digit"
                                                                        ...
                                                                        {!isCheckedOut &&
                                                                            <button onClick={() => handleCheckout(doc)}
                                                                                    title="Auschecken" style={{
                                                                                background: "none",
                                                                                border: "none",
                                                                                cursor: "pointer",
                                                                                color: s.textMuted,
                                                                                ...
                                                                                borderRadius: 4,
                                                                                flexShrink: 0
                                                                            }}><LockOpen size={13}/></button>}
                                                                        {isMyCheckout &&
                                                                            <button onClick={() => openCheckin(doc)}
                                                                                    title="Einchecken" style={{
                                                                                background: "none",
                                                                                border: "none",
                                                                                cursor: "pointer",
                                                                                color: accent,
                                                                        ...
                                                                        {lockedByOther && isAdmin && <button
                                                                            onClick={() => handleCheckin(doc, null)}
                                                                            title="Sperre aufheben" style={{
                                                                            background: "none",
                                                                            border: "none",
                                                                            cursor: "pointer",
                                                                            color: "#ef4444",
                                                                            ...
                                                                            borderRadius: 4,
                                                                            flexShrink: 0
                                                                        }}><ShieldAlert size={13}/></button>}
                                                                        <button onClick={() => setEditDoc(doc)}
                                                                                title="Bearbeiten" style={{
                                                                            background: "none",
                                                                            border: "none",
                                                                            cursor: "pointer",
                                                                            color: s.textMuted,
                                                                        ...
                                                                        <button onClick={() => setCopyDoc(doc)}
                                                                                title="Kopieren (neu verschlagworten)"
                                                                                style={{
                                                                                    background: "none",
                                                                                    border: "none",
                                                                                    cursor: "pointer",
                                                                                    ...
                                                                                    borderRadius: 4,
                                                                                    flexShrink: 0
                                                                                }}><CopyPlus size={13}/></button>
                                                                        <button onClick={() => setVersionsDoc(doc)}
                                                                                title="Versionsverlauf" style={{
                                                                            background: "none",
                                                                            border: "none",
                                                                            cursor: "pointer",
                                                                            color: s.textMuted,
                                                                        ...
                                                                        <button
                                                                            onClick={() => !lockedByOther && handleDelete(doc)}
                                                                            title={lockedByOther ? "Gesperrt" : "Löschen"}
                                                                            style={{
                                                                                background: "none",
                                                                                border: "none",
                                                                                cursor: lockedByOther ? "not-allowed" : "pointer",
                        ...
                        {new Date(doc.updated_at || doc.created_at).toLocaleDateString("de-CH", {
                            ...
                            year: "2-digit"
                                            ...
                                            {lockedByOther && isAdmin && (
                                                <button onClick={() => handleCheckin(doc, null)}
                                                        title="Sperre aufheben (Admin)"
                                                        style={{
                                                            background: "none",
                                                            border: "none",
                                                            cursor: "pointer",
