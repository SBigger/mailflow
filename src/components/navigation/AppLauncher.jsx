  ...
  return (
    ...
    >
      ...
      <span style={{ minWidth: 0 }}>
        <span style={{
          display: 'block', fontSize: big ? 13.5 : 12.5, fontWeight: 650, color: '#26302a',
...
export default function AppLauncher({ open, onClose, profile, dark = false }) {
  ...
  const onKeyDown = (e) => {
    ...
    else if (e.key === 'Enter') { e.preventDefault(); openApp(results[Math.min(sel, results.length - 1)]); }
  };
          ...
          {query.trim() ? (
            ...
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 8, marginBottom: 14 }}>
                {topApps.map(app => (
                  <AppTile key={`top-${app.groupId}-${app.label}`} app={app} onOpen={openApp} big />
                ))}
              </div>
