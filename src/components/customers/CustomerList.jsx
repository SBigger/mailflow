...
import {
  Search, Building2, UserRound, ChevronDown, Eye, EyeOff,
} from "lucide-react";
import {
...
export default function CustomerList({
  ...
  const inactiveCount = typeFiltered.filter(c => c.aktiv === false).length;

  // Apply search + inactive filter
  const filtered = typeFiltered.filter(c => {
    const matchSearch = (c.company_name || "").toLowerCase().includes(search.toLowerCase());
    const matchActive = showInactive ? true : c.aktiv !== false;
  ...
  return (
    <div className="flex flex-col h-full border-r" style={{ backgroundColor: isArtis ? '#ffffff' : isLight ? '#ffffff' : 'rgba(24,24,27,0.4)', borderColor: isArtis ? '#ccd8cc' : isLight ? '#d4d4e8' : 'rgba(63,63,70,0.6)' }}>
      ...
      {/* Inaktive-Toggle (nur wenn es inaktive gibt) */}
      {inactiveCount > 0 && (
        <div
          className="px-4 py-2 border-t flex items-center justify-center"
          style={{ borderColor: isArtis ? '#ccd8cc' : isLight ? '#d4d4e8' : 'rgba(63,63,70,0.6)' }}
        >
          <button
            onClick={() => setShowInactive(v => !v)}
            className="flex items-center gap-1.5 text-xs transition-colors"
            style={{ color: showInactive ? '#ef4444' : textMuted }}
          >
            {showInactive
              ? <><EyeOff className="h-3.5 w-3.5" /> Inaktive ausblenden</>
              : <><Eye className="h-3.5 w-3.5" /> {inactiveCount} Inaktive anzeigen</>
            }
          </button>
        </div>
      )}
